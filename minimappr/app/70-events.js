...
$("#btnPaste").addEventListener("click",()=>{ $("#pText").value=""; $("#pErr").textContent=""; openModal("pasteModal"); });
$("#pOk").addEventListener("click",()=>{
  ...
  closeModal("pasteModal"); startMapping(parsed, "Pasted data");
});
...
// column mapping
let pendingParsed=null, editingId=null;
...
function startMapping(parsed, name){
  ...
  parsed.columns.forEach(c=>{
    ...
    sel.value=auto[c]; sel.dataset.col=c;
...
$("#mapOk").addEventListener("click",()=>{
  ...
  addFromMapping(pendingParsed, mapping, $("#mapName").value.trim()||"Dataset");
  closeModal("mapModal");
});
function addFromMapping(parsed, mapping, name, baseMarker, groupBy){
  const {points, attrCols, bad}=pointsFromMapping(parsed, mapping);
  if(!points.length){ alert("No valid coordinates could be read from that data."); return; }
...
$("#btnManual").addEventListener("click",()=>{ editingId=null; fillSelect($("#mMarker"),MARKERS,"Circle");
  ...
  $("#mOrder").value="latlon"; $("#mErr").textContent=""; openModal("manualModal"); });
